...
<li class="{$style}{if $smarty.foreach.artikel.iteration % 3 == 0} nomargin{/if}{if $smarty.foreach.artikel.iteration % 3 == 0} mclear{/if}">
   <div class="pwrapper">
      ...
      <div class="pimage">
         <div class="image_overlay_wrapper" id="image_drag_article{$Artikel->kArtikel}">
            ...
            <a href="{$Artikel->cURL}" {if $Einstellungen.template.articleoverview.article_image_preview == 'Y'}class="image_preview"{/if} ref="{$Artikel->Bilder[0]->cPfadNormal}">
               ...
               <img src="{$Artikel->cVorschaubild}" {if $Einstellungen.template.articleoverview.article_show_image_attrib=='Y' && !empty($Artikel->Bilder[1]->cPfadKlein)}onmouseover="switchImageAttrib('image{$Artikel->kArtikel}_{$imgcounter}','{$Artikel->Bilder[1]->cPfadKlein}');" onmouseout="switchImageAttrib('image{$Artikel->kArtikel}_{$imgcounter}','{$Artikel->cVorschaubild}')" {/if}alt="{$Artikel->cName|strip_tags|escape:"quotes"|truncate:60}" id="image{$Artikel->kArtikel}_{$imgcounter}" class="image" />
            </a>
      ...
      <h3><img src="/bilder/intern/zero.gif" border="0" width="10" height="15"><br><a href="{$Artikel->cURL}">{$Artikel->cName}</a></h3>
      {* A Smarty comment - ODM, 12/08/2013, Biker City & All For SPS: Do Not Show Product Number if HAN is not blank in WAWI ERP *}
			 {if $Artikel->cHAN !=""}
				<font color="#000000"><strong>{lang key="productHAN" section="global"}: </strong><span id="hanArtNr">{$Artikel->cHAN}</span></font><p>&nbsp;</p>
			 {else}
				<font color="#000000"><strong><font color="#000000">{lang key="productNo" section="global"}: </strong> <span id="artnr">{$Artikel->cArtNr}</font></span><p>&nbsp;</p>
			{/if}
            ...
            <div class="article_info_wrapper">
               <ul class="article_info">
                  ...
                  {elseif $anzeige=='verfuegbarkeit' || $anzeige=='genau'}
                     <li><strong>{$Artikel->Lageranzeige->cLagerhinweis[$anzeige]}</strong></li>
