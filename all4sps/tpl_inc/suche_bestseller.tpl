<h3>{lang key="bestseller" section="global"}</h3>
<ul class="styled_view bestseller">
{foreach name=bestseller from=$oBestseller_arr item=Artikel}
<li class="gallery">
               ...
               <div class="article_price">
                  ...
                  <div class="article_buy">
                     ...
                     {if ($Artikel->inWarenkorbLegbar == 1 || ($Artikel->nErscheinendesProdukt == 1 && $Einstellungen.global.global_erscheinende_kaeuflich == "Y")) && $Artikel->nIstVater == 0 && $Artikel->Variationen|@count == 0 && !$Artikel->bHasKonfig}
                        <span><input type="text" onfocus="this.setAttribute('autocomplete', 'off');" id="quantity{$Artikel->kArtikel}" class="quantity" name="anzahl" {if $Artikel->fAbnahmeintervall > 1}value="{$Artikel->fAbnahmeintervall}" onblur="javascript:gibAbnahmeIntervall(this, {$Artikel->fAbnahmeintervall});"{else}value="1"{/if} /></span>
