...
<form id="warenkorb_form" method="post" action="warenkorb.php" class="form">
   ...
   <div id="contentmid">
      <div class="pageTitle"><h1>{$Warenkorbtext}</h1></div>
      ...
      {if ($Warenkorb->PositionenArr|@count > 0)}
         ...
         <div class="basket_wrapper">
            ...
            <div id="freegift" class="container">
                  ...
                  <h2 class="title">{lang key="freeGiftFromOrderValueBasket" section="global"}, {lang key="freeGiftFrom1" section="global"}...</h2>
                  <ul class="hlist articles">
                     {foreach name=gratisgeschenke from=$oArtikelGeschenk_arr item=oArtikelGeschenk}
                     <li class="p23 tcenter {if $smarty.foreach.gratisgeschenke.iteration % 4 == 0}nomargin {/if}{if $smarty.foreach.gratisgeschenke.index % 4 == 0}clear{/if}">
                        <div>
                           <form method="post" name="freegift" action="warenkorb.php">
                              <h4>{$oArtikelGeschenk->cBestellwert} {lang key="freeGiftFrom2" section="global"}</h4><br />
                                 <p class="pimage">
                                    <img src="{$oArtikelGeschenk->Bilder[0]->cPfadKlein}" class="image" />
                                 </p>
                                 
                                 <h3>{$oArtikelGeschenk->cName}</h3>

                              <input name="gratisgeschenk" type="hidden" value="{$oArtikelGeschenk->kArtikel}" id="gift{$oArtikelGeschenk->kArtikel}" />
                              <input type="hidden" name="{$session_name}" value="{$session_id}" />
            ...
            <div id="xselling" class="container">
               <h2 class="underline">{lang key="basketCustomerWhoBoughtXBoughtAlsoY" section="global"}</h2>
               
               <ul class="hlist articles">
               {foreach name=Xsell_artikel from=$xselling->Kauf->Artikel item=ArtikelXsell}
                  {counter assign=imgcounter print=0}
                  <li class="p23 {if $smarty.foreach.Xsell_artikel.iteration % 4 == 0}nomargin {/if}{if $smarty.foreach.Xsell_artikel.index % 4 == 0}clear{/if}">
                     <div class="pwrapper">
                     <p class="pimage">
                        <a href="{$ArtikelXsell->cURL}"><img alt="{$ArtikelXsell->cName}" src="{$ArtikelXsell->cVorschaubild}" class="image"></a>
                     </p>
                     <h3><a href="{$ArtikelXsell->cURL}">{$ArtikelXsell->cName}</a></h3>
                     
                     <div class="left p40">
                        <span class="stars p{$ArtikelXsell->fDurchschnittsBewertung|replace:'.':'_'}"></span>
                     </div>
