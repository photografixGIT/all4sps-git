...
<div id="article">
   ...
   <ul class="container hlist article_navigator">
      <li class="prev_article">
         {if $NavigationBlaettern->vorherigerArtikel->kArtikel}<a href="{$NavigationBlaettern->vorherigerArtikel->cURL}" title="{$NavigationBlaettern->vorherigerArtikel->cName}">{lang key="previousProduct" section="productDetails"}</a>{/if}
      </li>
      <li class="next_article">
         {if $NavigationBlaettern->naechsterArtikel->kArtikel}<a href="{$NavigationBlaettern->naechsterArtikel->cURL}" title="{$NavigationBlaettern->naechsterArtikel->cName}">{lang key="nextProduct" section="productDetails"}</a>{/if}
      </li>
   </ul>
   {/if}
   ...
   <div class="article_details">
      <div class="left p50">
         <ul class="article_list">
            ...
            <li><strong>{lang key="productNo" section="global"}:</strong> <span id="artnr">{$Artikel->cArtNr}</span></li>
            ...
            {if $Artikel->fDurchschnittsBewertung > 0}
               <li>
                  <a href="index.php?a={$Artikel->kArtikel}&amp;bewertung_anzeigen=1{if $SID}&amp;{$SID}{/if}#mytabset" title="{lang key="moreProductRatings" section="product rating"}">
   ...
   {if $Xselling->Standard}
      ...
      {foreach name=Xsell_gruppen from=$Xselling->Standard->XSellGruppen item=Gruppe}
         {if count($Gruppe->Artikel)>0}
            {include file="tpl_inc/artikel_inc_liste.tpl" headline=$Gruppe->Name oArtikel_arr=$Gruppe->Artikel cClass="article_list_of_items"}
         {/if}
      {/foreach}
      </div>
   {/if}
