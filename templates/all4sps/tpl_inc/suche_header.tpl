...
{if isset($oNavigationsinfo)}
   <div class="category_wrapper clearall">
      <ul class="category_item">
         ...
         {/if}
         <li class="desc">
            {if $oNavigationsinfo->cName}<h2>{$oNavigationsinfo->cName}</h2>{/if}
...
{if count($Suchergebnisse->Artikel->elemente)>0}
   ...
   <div class="container">
      <ul class="hlist">
         <li class="p50"><strong>{lang key="page" section="productOverview" alt_section="global,"} {$Suchergebnisse->Seitenzahlen->AktuelleSeite}</strong> {lang key="of" section="productOverview"} {$Suchergebnisse->Seitenzahlen->MaxSeiten}</li>
         <li class="p50 tright">{$Suchergebnisse->GesamtanzahlArtikel} {if $Suchergebnisse->GesamtanzahlArtikel>0}{lang key="products" section="global"}{else}{lang key="product" section="global"}{/if} {lang key="found" section="global"}, {lang key="showProducts" section="global"} {$Suchergebnisse->ArtikelVon} - {$Suchergebnisse->ArtikelBis}</li>
      </ul>
   </div>
   ...
   {if $Einstellungen.artikeluebersicht.suchfilter_anzeigen_ab == 0 || count($Suchergebnisse->Artikel->elemente) >= $Einstellungen.artikeluebersicht.suchfilter_anzeigen_ab}
      <fieldset>
         ...
         <div id="article_filter">
            {if $Einstellungen.navigationsfilter.allgemein_kategoriefilter_benutzen == "Y" && ($Suchergebnisse->Kategorieauswahl|@count > 1 || $NaviFilter->KategorieFilter->kKategorie > 0)}
            ...
            {if $Einstellungen.navigationsfilter.allgemein_herstellerfilter_benutzen == "Y" && !isset($oExtendedJTLSearchResponse)}
            ...
            {if $Einstellungen.navigationsfilter.merkmalfilter_verwenden == "content"}
               ...
               <div id="filter_group">
                  ...
                  <div class="item">
                     ...
                     <ul class="values">
                        {if $Suchergebnisse->Suchspecialauswahl[1]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[1]->cURL}">{lang key="bestsellers" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[1]->nAnzahl}){/if}</em></li>{/if}
                        {if $Suchergebnisse->Suchspecialauswahl[2]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[2]->cURL}">{lang key="specialOffer" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[2]->nAnzahl}){/if}</em></li>{/if}
                        {if $Suchergebnisse->Suchspecialauswahl[3]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[3]->cURL}">{lang key="newProducts" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[3]->nAnzahl}){/if}</em></li>{/if}
                        {if $Suchergebnisse->Suchspecialauswahl[4]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[4]->cURL}">{lang key="topOffer" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[4]->nAnzahl}){/if}</em></li>{/if}
                        {if $Suchergebnisse->Suchspecialauswahl[5]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[5]->cURL}">{lang key="upcomingProducts" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[5]->nAnzahl}){/if}</em></li>{/if}
                        {if $Suchergebnisse->Suchspecialauswahl[6]->nAnzahl > 0}<li> <a rel="nofollow" href="{$Suchergebnisse->Suchspecialauswahl[6]->cURL}">{lang key="topReviews" section="global"}</a> <em class="count">{if !$nMaxAnzahlArtikel}({$Suchergebnisse->Suchspecialauswahl[6]->nAnzahl}){/if}</em></li>{/if}
                     </ul>
                  </div>
                  {/if}
                  ...
                  <div class="item">
                     ...
                     <ul class="values">
                        {if $NaviFilter->SuchspecialFilter->kKey == 1}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="bestsellers" section="global"}</a></li>
                        {elseif $NaviFilter->SuchspecialFilter->kKey == 2}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="specialOffer" section="global"}</a></li>
                        {elseif $NaviFilter->SuchspecialFilter->kKey == 3}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="newProducts" section="global"}</a></li>
                        {elseif $NaviFilter->SuchspecialFilter->kKey == 4}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="topOffer" section="global"}</a></li>
                        {elseif $NaviFilter->SuchspecialFilter->kKey == 5}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="upcomingProducts" section="global"}</a></li>
                        {elseif $NaviFilter->SuchspecialFilter->kKey == 6}
                           <li class="selected"><a rel="nofollow" href="{$NaviFilter->URL->cAlleSuchspecials}" class="active">{lang key="topReviews" section="global"}</a></li>
                        {/if}
                     </ul>
                  </div>
                  {/if}
...
{if $KategorieInhalt->TopArtikel->elemente|@count >0}
   <div class="container">
      <h2>{lang key="topOffer" section="global"}</h2>
      <ul class="hlist articles">
      {foreach name=topartikel from=$KategorieInhalt->TopArtikel->elemente item=Artikel}
         <li class="p33 tcenter {if $smarty.foreach.topartikel.index%3==0}clear{/if}">
            <div>
            <p>
               <a href="{$Artikel->cURL}">
            ...
            {if $smarty.session.Kundengruppe->darfPreiseSehen}
               <p>
                  <span class="price_label">{lang key="only" section="global"}</span> <span class="price">{if $Artikel->Preise->fVKNetto==0 && $Einstellungen.global.global_preis0=="N"}{lang key="priceOnApplication" section="global"}{else}{$Artikel->Preise->cVKLocalized[$NettoPreise]}{/if}</span>
               </p>
...
{if $KategorieInhalt->BestsellerArtikel->elemente|@count >0}
   <div class="container">
      <h2>{lang key="bestsellers" section="global"}</h2>
      <ul class="hlist articles">
